...
export async function fetchNewColor() {
  ...
  // Viel kürzerer und cooler Weg:
  const dataX = await fetch(colorApiUrl).then((response) => response.json());
  console.log("Coole kurze Methode data zu fetchen: ", dataX);
  console.log(dataX.hex.value);
  console.log(dataX.name.value);
